<template>
    <div class="flex items-center justify-center p-2 rounded-full bg-gray-100 hover:bg-gray-200 border-2 border-gray-200 hover:border-gray-300 transition cursor-pointer w-24 h-24" @click="triggerUploadInput">
        <input class="hidden" type="file" accept="image/*" ref="uploadInput" @change="handleInputChange" />
        <UploadIcon v-if="!uploadedFile" class="text-gray-400 w-10 h-10" />
        <img v-else class="object-scale-down rounded-full" :src="uploadedFile" alt="Avatar" />
    </div>
</template>
