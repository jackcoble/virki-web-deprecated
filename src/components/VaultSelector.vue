                ...
                <div class="mt-2 border-t border-b border-gray-200 divide-y divide-gray-200">
                    <div v-for="(vault) in vaults" :key="vault.id" class="relative flex items-start py-3">
                        <div class="mr-3 flex items-center h-5">
                            <input type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" :checked="vault.id === activeVault!.id" />
                        </div>
...
export default defineComponent({
    ...
    setup() {
        ...
        const activeVault = ref({} as EncryptedVault);

        // Watch the vault store for a change to the active vault ID
        watch(
            () => vaultStore.getActiveVaultId,
            (id, prevId) => {
                if (id !== prevId) {
                    // Assume vault has been changed, so update active vault
                    activeVault.value = vaultStore.getActiveVault!;
                }
            }
        )
